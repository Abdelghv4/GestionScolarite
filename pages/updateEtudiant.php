<?php
  require_once('identifier.php');
    ...
    $code_massar=isset($_POST['code_massar'])?$_POST['code_massar']:"";
    ...
    if(!empty($nomPhoto)){
        $requete="update etudiant set nom=?,prenom=?,civilite=?,idFiliere=?,photo=?, date_naissance=? ,lieu_naissance=? ,adresse=?
         ,ville=? ,cin=?,tel_domicile=? ,tel_professionnel=?
          ...
          ,date_inscription=? ,code_massar=? ,resultat=? ,annee_scolaire=?  where idEtudiant=?";
        $params=array($nom,$prenom,$civilite,$idFiliere,$nomPhoto,$date_naissance,$lieu_naissance ,$adresse ,$ville
         ,$cin,$tel_domicile ,$tel_professionnel ,$niveau_scolaire ,$gmail  ,$dernier_etablissement ,$num_inscription 
        ,$date_inscription ,$code_massar ,$resultat ,$annee_scolaire,$idE );
    }else{
        ...
        ,niveau_scolaire=? ,gmail=?  ,dernier_etablissement=? ,num_inscription=?
         ,date_inscription=? ,code_massar=? ,resultat=? ,annee_scolaire=?  where idEtudiant=?";
        $params=array($nom,$prenom,$civilite,$idFiliere,$idS,$date_naissance,$lieu_naissance 
        ,$adresse ,$ville ,$cin,$tel_domicile ,$tel_professionnel ,$niveau_scolaire 
        ,$gmail  ,$dernier_etablissement ,$num_inscription 
        ,$date_inscription ,$code_massar ,$resultat ,$annee_scolaire );
    }
